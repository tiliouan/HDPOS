<?php
/**
 * Plugin Name: HardWarePOS Point of Sale for WooCommerce
 * Description: <code><strong>HardWarePOS Point of Sale for WooCommerce</strong></code> allows you to turn your WooCommerce installation into an easy to use and powerful cash register for each type of store or business. <a href="https://lmdelm.com/" target="_blank">Get more plugins for your e-commerce shop on <strong>LMDELM</strong></a>
 * Author: LMDELM
 * Text Domain: hardwarepos-point-of-sale-for-woocommerce
 * Domain Path: /languages/
 * Version: 3.14.0
 * Author URI: https://lmdelm.com/
 ...
 *
 * @author  LMDELM <plugins@lmdelm.com>
 * @package HardWarePOS Point of Sale for WooCommerce
 * @version 3.14.0
 */
	...
	function yith_pos_install_woocommerce_admin_notice() {
		...
		<div class="error">
			<p>
				...
				echo sprintf( esc_html__( '%s is enabled but not effective. It requires WooCommerce in order to work.', 'hardwarepos-point-of-sale-for-woocommerce' ), esc_html( YITH_POS_PLUGIN_NAME ) );
				?>
			</p>
		</div>
		<?php
...
! defined( 'YITH_POS_SLUG' ) && define( 'YITH_POS_SLUG', 'hardwarepos-point-of-sale-for-woocommerce' );
! defined( 'YITH_POS_SECRET_KEY' ) && define( 'YITH_POS_SECRET_KEY', '' );
! defined( 'YITH_POS_PLUGIN_NAME' ) && define( 'YITH_POS_PLUGIN_NAME', 'HardWarePOS Point of Sale for WooCommerce' );
...
if ( ! function_exists( 'yith_pos_init' ) ) {
	...
	function yith_pos_init() {
		if ( function_exists( 'yith_plugin_fw_load_plugin_textdomain' ) ) {
			yith_plugin_fw_load_plugin_textdomain( 'hardwarepos-point-of-sale-for-woocommerce', basename( dirname( __FILE__ ) ) . '/languages' );
		}
